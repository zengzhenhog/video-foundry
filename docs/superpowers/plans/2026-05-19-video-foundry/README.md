# video-foundry AI 分步执行计划索引

日期：2026-05-19  
来源设计：`docs/superpowers/specs/2026-05-19-video-foundry-development-plan-design.md`  
系统设计：`docs/system-design.md`、`docs/system-design.zh-CN.md`

本目录把 video-foundry 开发计划拆成可单独交给 AI 执行的阶段计划文件。每个阶段文件都尽量自包含，包含目标、允许修改范围、必须新增或修改的文件、禁止事项、原子执行步骤、验收标准和验证命令。
...
```text
请执行 docs/superpowers/plans/2026-05-19-video-foundry/phase-XX-*.md。
...
- 新增 API 路由时，必须同时完成路由注册。可以在 `src/video_foundry/api/main.py` 中注册，也可以在 Phase 1 建立集中 router 聚合；后续阶段沿用同一模式。
